...
<head>
  ...
  <title>Tambah Bioskop &mdash; Dakota</title>
          ...
          <ul class="sidebar-menu">
            ...
            <li class="dropdown active">
              ...
              <ul class="dropdown-menu">
                ...
                <li class="active"><a class="nav-link" href="databioskop.php">Data Bioskop</a></li>
              </ul>
            </li>
                ...
                <div class="card">
                  <form method="POST">
                    ...
                    <div class="card-footer text-right">
                      ...
                      <a href="databioskop.php" class="btn btn-secondary">Kembali</a>
                    </div>
                  </form>
                </div>
              </div>
...
if (isset($_POST['Submit'])) {
  ...
  $alamat = $_POST['alamat'];
  $nohp = $_POST['nohp'];
  $query_cek = mysqli_query($konek, "select * from bioskop where nama_cabang = '$nama_cabang'");
  $cek = mysqli_num_rows($query_cek);
  if (empty($nama_cabang) || empty($alamat) || empty($nohp)) {
    echo "<script>
    swal('Oops!', 'Data anda kosong', 'error');</script>";
  } else if($cek==0){
    $sql = mysqli_query($konek, "INSERT INTO bioskop (nama_cabang,alamat,nohp) VALUES ('$nama_cabang','$alamat','$nohp')");
    echo "<script>
      swal({title: 'Tambah Data Berhasil',text: 'Data telah ditambah',icon: 'success',confirmButtonText: 'OK'
      }).then((result) => {if (result.value){
          window.location = 'databioskop.php';
          }
      })</script>";
  }else{
    echo "<script>
    swal('Oops!', 'Data anda sama', 'error');</script>";
  }
}
?>