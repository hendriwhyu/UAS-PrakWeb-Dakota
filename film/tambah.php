...
<head>
  ...
  <title>Tambah Film &mdash; Dakota</title>
...
if (isset($_POST['Submit'])) {
  $nama_film = $_POST['film'];
  $genre = $_POST['genre'];
  $tahun_rilis = $_POST['tahun_rilis'];
  $query_cek = mysqli_query($konek, "select * from film where nama_film = '$nama_film'");
  $cek = mysqli_num_rows($query_cek);
  if (empty($nama_film) || empty($genre) || empty($tahun_rilis)) {
    echo "<script>
    swal('Oops!', 'Data anda kosong', 'error');</script>";
  } else if ($cek == 0) {
    $sql = mysqli_query($konek, "INSERT INTO film (nama_film,genre,tahun_rilis) VALUES ('$nama_film','$genre','$tahun_rilis')");
    echo "<script>
